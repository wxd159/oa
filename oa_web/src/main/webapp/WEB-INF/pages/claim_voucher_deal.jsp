                ...
                <div class="panel-menu">
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-md-9 text-right">
                            <div class="btn-group">
                                <button type="button" class="btn btn-default light">
                                    <i class="fa fa-chevron-left" onclick="javascript:window.location.href='/claim_voucher/deal?page=${sessionScope.pageInfo-1}';"></i>
                                </button>
                                <button type="button" class="btn btn-default light">
                                    <i class="fa fa-chevron-right" onclick="javascript:window.location.href='/claim_voucher/deal?page=${sessionScope.pageInfo+1}';"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="panel-body pn">
                    <table id="message-table" class="table admin-form theme-warning tc-checkbox-1">
                        ...
                        <tr class="message-unread">
                            ...
                            <td>
                                <c:if test="${cv.status==Contant.CREATED || cv.status==Contant.BACK}">
                                    <a href="/claim_voucher/to_update?id=${cv.id}">修改</a>
                                    <a href="/claim_voucher/submit?id=${cv.id}">提交</a>
                                </c:if>
                                <c:if test="${cv.status==Contant.SUBMIT || cv.status==Contant.RECHECK}">
                                    <a href="/claim_voucher/to_check?id=${cv.id}">审核</a>
                                </c:if>
                                <c:if test="${cv.status==Contant.APPROVED}">
